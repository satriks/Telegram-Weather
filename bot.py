...
from setting import bot_config
...
class ChoisCityWeather(StatesGroup):
    waiting_city = State()
class SetUserCity(StatesGroup):
    user_city = State()

@dp.message_handler(regexp='Погода в другом месте')
async  def city_start(messege: types.Message):

    markup = types.reply_keyboard.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    btn1 = types.KeyboardButton('Меню')
    markup.add(btn1)
    text = 'Введите название города'
    await messege.answer(text, reply_markup=markup)
    await ChoisCityWeather.waiting_city.set()

@dp.message_handler(commands=['start'])
async  def start_massage(messege: types.Message):
    orm.add_user(messege.from_user.id)
    ...
    text = f'привет {messege.from_user.first_name}, я бот что расскажет тебе о погоде на сегодня '
    await messege.answer(text, reply_markup=markup)
...
async  def start_massage(messege):
    markup = types.reply_keyboard.ReplyKeyboardMarkup(row_width=2)
    btn1 = types.KeyboardButton('Погода в моем городе')
    btn2 = types.KeyboardButton('Погода в другом месте')
    btn3 = types.KeyboardButton('История')
...
@dp.message_handler(state=ChoisCityWeather.waiting_city)
async def сity_chosen(message: types.Message, state: FSMContext):
    if message.text[0].islower():
        await  message.answer('Город начинается с большой буквы ')
        return
...
async  def city_start(messege: types.Message):
    ...
    text = 'Введите название города'
    await messege.answer(text, reply_markup=markup)
    await SetUserCity.user_city.set()

@dp.message_handler(state=SetUserCity.user_city)
async def сity_chosen(message: types.Message, state: FSMContext):
    if message.text[0].islower():
        await  message.answer('Город начинается с большой буквы ')
        return
    ...
    orm.set_user_city(message.from_user.id, user_data.get('user_city'))
    markup = types.reply_keyboard.ReplyKeyboardMarkup(row_width=2)
    btn1 = types.KeyboardButton('Погода в моем городе')
    btn2 = types.KeyboardButton('Погода в другом месте')
    btn3 = types.KeyboardButton('История')
    btn4 = types.KeyboardButton('Установить свой город')
    markup.add(btn1, btn2, btn3, btn4)
    text = f'Город установлен : {user_data.get("user_city")}'
    await message.answer(text, reply_markup=markup)
    await state.finish()
...
async def get_reports(message: types.Message):
    ...
    for report in reports[:current_page*4]:
        inline_markup.add(types.InlineKeyboardButton(
            ...
            callback_data=f'report_{report.wr_id}'
        ))
    current_page += 1
    inline_markup.row(
...
async def get_all_users(message: types.Message):
    ...
    for user in users[:current_page*4]:
        inline_markup.add(types.InlineKeyboardButton(
            text=f'{user.user_id}) id: {user.tg_id} Подключился: {user.connection_date.day}.{user.connection_date.month}.{user.connection_date.year} Отчётов: {len(user.reports)}',
